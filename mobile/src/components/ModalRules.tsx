...
export function ModalRules({ showModal, setShowModal }: ModalProps) {
  return (
    <Modal isOpen={showModal} onClose={() => setShowModal(false)} size="lg">
      <Modal.Content maxWidth="350">
        <Modal.CloseButton />
        <Modal.Header>Regras do Bolão</Modal.Header>
        <Modal.Body>
          <HStack alignItems="center" justifyContent="space-between">
            <Text fontWeight="medium" mb="4">
              As regras da pontuação são distribuídas da seguinte forma:{" "}
            </Text>
          </HStack>
          <VStack space={3}>
            <HStack alignItems="center" justifyContent="space-between">
